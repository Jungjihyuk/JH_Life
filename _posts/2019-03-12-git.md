...
> 브랜치 생성 및 삭제 
  git branch bugFix (현재 HEAD는 master) => 생성 
  git checkout -b bugFix => 생성과 동시에 HEAD옮기기 
  git branch --delete bugFix => 삭제
  git branch -D bugFix => 브랜치에 commit이 남아 있을때 강제 삭제
  git push origin :bugFix => 원격저장소의 브랜치 삭제 
